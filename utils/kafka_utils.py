...
class MyProcess:
    ...
    def __init__(self, config):
        ...
        self.origin_data['timestamp'] = int(time.time())
    ...
    def websocket_process(self, websocket_queue):
        ...
        while True:
            try:
                ...
                while True:
                    try:
                        data = websocket_queue.get_nowait()
                    except:
                        print('websocket queue is empty')
                        continue
        ...
        while True:
            try:
                ...
                for message in consumer:
                    ...
                    data = {}
                    if message.get('positionId') not in ["CD114", "CD116"]:
                        continue
                    for obj in message.get('objs', []):
                        if not obj.get('id'):
                            continue
                        obj["vehPlateString"] = str(obj["id"])
                    ...
                    if len(temp_data) < max_size:  # 防止数据量过大
                        ...
                        # print('after deal', len(origin_data['data']), origin_data['timestamp'])
            except:
                error = str(traceback.format_exc())
