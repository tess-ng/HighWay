...
from utils.config import neighbor_distance
import json 
...
#        if vehiStatus:
#            mColor = get_attr(vehiStatus, 'mColor')
#            mPoint = get_attr(vehiStatus, 'mPoint')
#            # print(mColor, [mColor.red(), mColor.green(), mColor.blue()])
#            data['data'].append(
#                {
#                    'id': get_attr(vehi, 'id'),
#                    'acc': get_attr(vehi, 'acce'),
#                    'color': [mColor.red(), mColor.green(), mColor.blue()],
#                    'lon': (0.038069 * mPoint.x() + 399.59) / 813.655 * 0.008152965789363975 + 121.53809584889734,
#                    'lat': (-0.038069 * mPoint.y() - 381.369) / 724.047 * 0.006200007257248785 + 31.25962330074992,
#                    'angle': get_attr(vehi, 'angle'),
#                    'position': [mPoint.x(), mPoint.y(), mPoint.z()],
#                    'distance': get_attr(vehiStatus, 'mrDrivDistance'),
#                    'speed': get_attr(vehiStatus, 'mrSpeed'),
#                    'vehiType': get_attr(vehiStatus, 'vehiType'),
#                    'lane': lane_convert(vehi.roadIsLink(), vehi.roadId(), vehi.lane().number()),
...
#            )
    ...
    for vehi in lAllVehi:
        ...
        if vehiStatus:
            ...
            origin_data.update(
                {
                    'longitude': int(((0.460792*mPoint.x()-1300.9)/(-394.137-1300.9)*(121.53749958491585-121.55412756)+121.55412756-0.0002) * 10000000),
                    'latitude': int(((-0.460792*mPoint.y()+1234.61)/(98.4579+1234.61)*(31.26049146808565-31.24923271 )+31.24923271) * 10000000),
                    "laneId": lane_convert(vehi.roadIsLink(),vehi.roadId(),vehi.lane().number()),
                    "angleGps": int(get_attr(vehi, 'angle') * 10),
                   # 'speed': json.loads(vehi.name()).get('speed', 20) * 100,
                    "sim_speed": p2m(get_attr(vehi, 'currSpeed')),
                }
            )
            # keys = ['id', 'type', 'score', 'longitude', 'latitude', 'angleGps', 'speed', 'vehPlateString', 'vehPlateColor', 'vehColor', 'vehType', 'laneId', 'laneType']
            data[0]['objs'].append(origin_data) 
    return data


def lane_convert(is_link, roadid, lane_id):  # tess车道编号从0开始
    if is_link:
        if roadid in [22, 21, 24, 25, 26, 238, 27, 28]:  # 0,1,2 -> 6,7,8
            lane_id = lane_id + 6
        elif roadid in [32, 31, 29]:  # 0,1->9,10
            lane_id = lane_id + 9
        elif roadid == 33:  # 0,1->6,7
            lane_id = lane_id + 6
        elif roadid in [18, 16, 15, 14, 13, 10, 11, 12]:  # 0,1,2->1,2,3
            lane_id = lane_id + 1
        elif roadid in [20, 34, 17, 19]:  # 0,1->4,5
            lane_id = lane_id + 4
        else:
            lane_id = 0
    else:
        if roadid in [15, 16, 17, 18, 230, 19, 20]:  # 0,1,2 -> 6,7,8
            lane_id = lane_id + 6
        elif roadid in [27, 26, 23]:  # 0,1->9,10
            lane_id = lane_id + 9
        elif roadid == 28:  # 0,1->6,7
            lane_id = lane_id + 6
        elif roadid in [9, 10, 8, 7, 6, 4, 13]:  # 0,1,2->1,2,3
            lane_id = lane_id + 1
        elif roadid in [14, 11, 12, 29]:  # 0,1->4,5
            lane_id = lane_id + 4
        else:
            lane_id = 0
